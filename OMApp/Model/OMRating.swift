...
enum OMRating: String, CaseIterable {
    case f = "F"
    case dMinus = "D-"
    case d = "D"
    case cMinus = "C-"
    ...
    static func fromScore(_ score: Int) -> OMRating {
        switch score {
        case 0..<100: return .f
...
struct LongevityScoreBreakdown {
    ...
    var rating: OMRating {
        return OMRating.fromScore(totalScore)
    }
    
    var nextRating: OMRating? {
        let currentIndex = OMRating.allCases.firstIndex(of: rating) ?? 0
        let nextIndex = currentIndex + 1
        return nextIndex < OMRating.allCases.count ? OMRating.allCases[nextIndex] : nil
    }
    ...
    private func scoreForRating(_ rating: OMRating) -> Int {
        switch rating {
        case .f: return 0
